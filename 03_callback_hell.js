...
const placeOrder = () => {
  ...
  getUser((error, waiter) => {
    if (error) {
      ...
      // run the get order here because it's only in this block that waiter is in scope

      getOrder((error, order) => {
        if (error) {
          console.log(`Error: ${error}`);
        } else {
          // run the getUser here because it's only in this block that waiter and order is in scope
          ...
          getUser((error, customer) => {
            if (error) {
              ...
              console.log(`${waiter} is delivering the ${order} to ${customer}`)
            }
          });
        }
      });
