  ...
  getUser((err, user1) => {
    ...
    getOrder((err, meal) => {
      ...
      getUser((err, user2) => {
        ...
        // Edge case
        if (user1 === user2) {
          console.log('Waiter cannot serve themselves!');
          return;
        }

        // we have access to user1, meal, user2 at the same time inhere
        console.log(`${user1} has taken the order of ${user2}`);
        console.log(`${user1} has delivered a ${meal} to ${user2}`);
      });
    });
  });
